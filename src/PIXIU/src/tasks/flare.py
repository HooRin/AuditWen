...
class Classification(Task):
    ...
    def process_results(self, doc, results):
        ...
        # index_of = ini_result.find('\n')
        # if index_of != -1:
        #     result = ini_result[:index_of]
        # else:
        #     result = ini_result
...
class AbstractiveSummarization(Task):
    ...
    def bart_score(self, items):
        golds, preds = zip(*items)
        bart_scorer = BARTScorer(device="cuda", checkpoint="/root/autodl-tmp/PIXIU/chancefocus/bart-large-cnn")
        bart_scorer.load(path="/root/autodl-tmp/PIXIU/src/metrics/BARTScore/bart_score.pth")
...
class FinQA(QA):
    DATASET_PATH = "/root/autodl-tmp/PIXIU/data/chancefocus/FinQA"
...
class ZHFinNL(Classification):
    DATASET_PATH = "/root/autodl-tmp/PIXIU/data/zh-audit-nl"
...
class ZHFinRE(Classification):
    DATASET_PATH = "/root/autodl-tmp/PIXIU/data/zh-audit-re"
...
class ZHFinEval(Classification):
    DATASET_PATH = "/root/autodl-tmp/PIXIU/data/zh-audit-eval"
...
class ZHFinQA(QA):
    DATASET_PATH = "/root/autodl-tmp/PIXIU/data/zh-audit-qa"
...
class ZHNER(NER):
    DATASET_PATH = "/root/autodl-tmp/PIXIU/data/zh-audit-ner"
...
class QAwithString(Task):
    ...
    def bart_score(self, items):
        golds, preds = zip(*items)
        bart_scorer = BARTScorer(device="cuda", checkpoint="/root/autodl-tmp/PIXIU/chancefocus/bart-base-chinese")
        #bart_scorer.load(path="/root/autodl-tmp/PIXIU/src/metrics/BARTScore/bart_score.pth")
        res = bart_scorer.score(srcs=preds, tgts=golds, batch_size=8)
        return sum(res) / len(res)
        #return res
...
class AuditNL(Classification):
    DATASET_PATH = "/.."

class AuditRE(Classification):
    DATASET_PATH = "/.."

class AuditQA(QAwithString):
    DATASET_PATH ="/.."